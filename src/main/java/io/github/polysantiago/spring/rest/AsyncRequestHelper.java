...
class AsyncRequestHelper {
  ...
  private static <T> ListenableFuture<ResponseEntity<Optional<T>>> toOptional(
      ListenableFuture<? extends ResponseEntity<?>> future) {
  ...
  <T> ListenableFuture<?> executeAsyncRequest(Method method, RequestEntity<T> requestEntity) {
    ResolvableType resolvedType =
        ResolvableType.forMethodReturnType(method, implementingClass).getGeneric(0);
    ...
    if (ResolvableTypeUtils.typeIsAnyOf(resolvedType, HttpEntity.class, ResponseEntity.class)) {
      SyntheticParametrizedTypeReference<T> responseType =
          fromResolvableType(resolvedType.getGeneric(0));
      return sendAsyncRequest(requestEntity, responseType);
    }

    ListenableFuture<ResponseEntity<T>> listenableFuture =
        sendAsyncRequest(requestEntity, wrappedReturnType);
  ...
  private <T> ListenableFuture<ResponseEntity<T>> sendAsyncRequest(
      RequestEntity<T> requestEntity, SyntheticParametrizedTypeReference<T> responseType) {
    return asyncRestTemplate.exchange(
        requestEntity.getUrl(), requestEntity.getMethod(), requestEntity, responseType);
  }
}
