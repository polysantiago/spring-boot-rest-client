...
class SyncRequestHelper {
  ...
  @Setter private boolean retryEnabled;
  ...
  private RuntimeException handleRuntimeException(RuntimeException ex) {
    if (retryEnabled
        && anyMatch(
            specification.getRetryableExceptions(),
            clazz -> clazz.isInstance(ex) || clazz.isInstance(getRootCause(ex)))) {
  ...
  private <T> Optional<T> handleHttpStatusCodeException(Method method, HttpStatusCodeException ex) {
    ...
    if (ResolvableTypeUtils.returnTypeIs(method, Optional.class)
  ...
  private <T> Object executeRequestInternal(
      MethodInvocation invocation, RequestEntity<T> requestEntity) {
    ...
    SyntheticParametrizedTypeReference<T> responseType =
        SyntheticParametrizedTypeReference.fromResolvableType(resolvedType);
  ...
  private <T> ResponseEntity<T> exchangeForResponseEntity(
      ResolvableType resolvedType, RequestEntity<T> requestEntity) {
    return restTemplate.exchange(
        requestEntity,
        SyntheticParametrizedTypeReference.fromResolvableType(resolvedType.getGeneric(0)));
  }
